...
namespace WebApi.Controllers
{
    public class StylistController : BaseController
    {
        ...
        [ProducesResponseType(400, Type = typeof(Result<object>))]
        public async Task<IActionResult> GetAvailableStylist([FromQuery] DateTime bookingDate, [FromQuery] int bookingHour, [FromQuery] int bookingMinute)
        {
            var bookingTime = new TimeSpan(bookingHour, bookingMinute, 0);

            var result = await _userService.GetAvailableStylists(bookingDate, bookingTime);
            return Ok(result);
        }
